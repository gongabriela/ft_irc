...
std::vector<std::string> UserCommand::execute(Client& client, const ParsedCommand& cmd) {
    ...
    if (client.isAuthenticated()) {
        replies.push_back(":" + serverName + " " + ERR_ALREADYREGISTRED_CODE + " * :" + ERR_ALREADYREGISTRED_MSG);
        return replies;
    }

    // USER command arguments: <user> <mode> <unused> <realname>
    if (cmd.args.size() < 4) {
        replies.push_back(":" + serverName + " " + ERR_NEEDMOREPARAMS_CODE + " USER :" + ERR_NEEDMOREPARAMS_MSG);
        return replies;
    }
