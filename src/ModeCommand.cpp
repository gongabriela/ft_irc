...
std::vector<std::string> ModeCommand::execute(Client& client, const ParsedCommand& cmd) {
    ...
    if (target[0] != '#' && target[0] != '&') return replies;
...
void ModeCommand::parseAndApplyModes(Client& client, Channel* channel, const ParsedCommand& cmd, std::string& changes, std::string& argsAppended) {
    ...
    for (size_t i = 0; i < modeString.length(); ++i) {
        ...
        if (flag == '+') { adding = true; if (changes.empty() || changes[changes.length()-1] != '+') changes += "+"; }
        else if (flag == '-') { adding = false; if (changes.empty() || changes[changes.length()-1] != '-') changes += "-"; }
        else if (flag == 'i') { handleInviteMode(channel, adding, changes); }
        else if (flag == 't') { handleTopicMode(channel, adding, changes); }
        else if (flag == 'k') {
            if (adding && argIndex < cmd.args.size()) { handleKeyMode(channel, adding, cmd.args[argIndex++], changes, argsAppended); }
            else if (!adding) { handleKeyMode(channel, adding, "", changes, argsAppended); }
        }
        else if (flag == 'l') {
            if (adding && argIndex < cmd.args.size()) { handleLimitMode(channel, adding, cmd.args[argIndex++], changes, argsAppended); }
            else if (!adding) { handleLimitMode(channel, adding, "", changes, argsAppended); }
        }
        else if (flag == 'o') {
            if (argIndex < cmd.args.size()) { handleOperatorMode(client, channel, adding, cmd.args[argIndex++], changes, argsAppended); }
        }
        else {
            client.queueMessage(_server.buildReply(ERR_UNKNOWNMODE_CODE, client.getNickname(), std::string(1, flag), "is unknown mode char to me"));
        }
    }
...
void ModeCommand::handleOperatorMode(Client& client, Channel* channel, bool adding, const std::string& targetNick, std::string& changes, std::string& argsAppended) {
    ...
    if (!target || !channel->isMember(target)) return; 
